...
const static TCHAR szzHardwareIDs[] = TEXT("Wintun");
...
get_net_interface_guid(
    ...
    while (iNumAttempts > 0) {
        ...
        HRESULT hr = CLSIDFromString((const WCHAR *)szCfgGuidString, (LPCLSID)pguidInterface);
        dwResult = SUCCEEDED(hr) ? ERROR_SUCCESS : ERROR_INVALID_DATA;
...
tap_create_interface(
    ...
    if (pbRebootRequired == NULL || pguidInterface == NULL) {
        return ERROR_BAD_ARGUMENTS;
    }
    ...
    if (hDevInfoList == INVALID_HANDLE_VALUE) {
        dwResult = GetLastError();
        msg(M_NONFATAL | M_ERRNO, "%s: SetupDiCreateDeviceInfoList failed", __FUNCTION__);
        return dwResult;
    }
    ...
    if (!SetupDiClassNameFromGuid(
            ...
            NULL)) {
        dwResult = GetLastError();
        msg(M_NONFATAL | M_ERRNO, "%s: SetupDiClassNameFromGuid failed", __FUNCTION__);
    ...
    if (!SetupDiCreateDeviceInfo(
            ...
            &devinfo_data)) {
        dwResult = GetLastError();
        msg(M_NONFATAL | M_ERRNO, "%s: SetupDiCreateDeviceInfo failed", __FUNCTION__);
        goto cleanup_hDevInfoList;
    }
    ...
    if (!SetupDiSetSelectedDevice(
            ...
            &devinfo_data)) {
        dwResult = GetLastError();
        msg(M_NONFATAL | M_ERRNO, "%s: SetupDiSetSelectedDevice failed", __FUNCTION__);
        goto cleanup_hDevInfoList;
    }
    ...
    if (!SetupDiSetDeviceRegistryProperty(
        ...
        dwResult = GetLastError();
        msg(M_NONFATAL | M_ERRNO, "%s: SetupDiSetDeviceRegistryProperty failed", __FUNCTION__);
        goto cleanup_hDevInfoList;
    }
    ...
    if (!SetupDiCallClassInstaller(
            ...
            &devinfo_data)) {
        dwResult = GetLastError();
        msg(M_NONFATAL | M_ERRNO, "%s: SetupDiCallClassInstaller(DIF_REGISTERDEVICE) failed", __FUNCTION__);
        goto cleanup_DriverInfoList;
    }
